...
```python
def to_imp(american):
    """American odds → implied probability (vig-free NOT required here — raw implied)."""    if american is None: return None
    p = float(american)
    return abs(p) / (abs(p) + 100) if p < 0 else 100 / (p + 100)

bet_imp   = to_imp(bet_price)       # e.g. +108 → 0.481
close_imp = to_imp(closing_line)    # e.g. +100 → 0.500

clv_pct = (close_imp - bet_imp) * 100
# Positive = closing implied prob HIGHER than bet implied prob = market moved your way ✅
# Negative = closing implied prob LOWER = market moved against you ❌

# This formula matches clv_update.py (calc_clv function) and MODEL_CORE Section 17.
# Do not use decimal-odds percentage change — it produces different results at extremes.
```
...
| `BET_LOG.md` | `/BET_LOG.md` | Human-readable companion to bets.json. Auto-generated by update-clv.yml — **do not edit manually**. If BET_LOG.md and bets.json disagree, bets.json wins. Never load BET_LOG.md at session start; use bets.json directly. |
