...
53. **[NEW — May 28] CLV must be pulled via web search at settlement — never estimated, never skipped.** At settlement, Claude web searches the Pinnacle closing line for each bet (Action Network and OddsPortal are acceptable fallbacks). This is automatic within the settlement session. Settle within 48 hours — closing line data degrades after that and may be unrecoverable. If no closing line is found after searching all sources: log `closingLine: null`, `clv: null`. Never fabricate or estimate. Future upgrade path: The Odds API paid tier when model is profitable. See DATA_SOURCES.md Closing Line Pull section for full protocol.
