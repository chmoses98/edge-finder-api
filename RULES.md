...
53. **[NEW — May 28] Closing line must be pulled from The Odds API (Pinnacle) at settlement — not estimated.** CLV is computed against the true Pinnacle closing line captured within 15 minutes of first pitch via The Odds API historical endpoint. Never estimate or approximate a closing line. If unavailable, log `closingLine: null` and `clv: null`. A fabricated or estimated closing line corrupts the evaluation dataset permanently. Paid Odds API tier is required infrastructure — this is not optional. See DATA_SOURCES.md Closing Line Pull section for full protocol.
