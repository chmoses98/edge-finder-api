...
| F5 winner (incl. Tie) | `KXMLBF5` | `KXMLBF5-26JUL302140BOSATH-BOS` / `-TIE` | First 5 innings | Yes | Yes | Team legs: Yes. **Tie leg: no** — `REQUIRED_MARKETS` has no `F5_Tie` entry | Team legs: Yes; Tie leg: fetched only (`odds.kalshi.f5ml.tie`) | Team legs: Yes; Tie: **No** | Yes (team legs) | Team legs: Yes | Team legs **FULLY_SUPPORTED**; Tie leg **MODELED_NOT_EXPOSED** (math exists in `lib/research/three_way_projection.py`, unused in production) |
