...
| `importBatchId` / `sourceRow` | Identity components for a timestamp-free bet (see §6.2). |
...
- `entryTimestamp` unknown → `sha1('bet_import', importBatchId, sourceRow, marketTicker, side)`.
- Neither → the pre-existing ULID-style fallback token (unchanged, rare).

`importBatchId` defaults to a content hash of the submitted rows
(`ids.build_import_batch_id`), so re-running the identical import file
is automatically a no-op **without** the caller having to track its own
batch IDs — a genuinely different resubmission (different stake/price/
etc.) naturally hashes to a different batch, and two rows in the *same*
batch are distinguished by `sourceRow` (their index) even if every other
field is identical, so two real tranches placed in one submission are
never merged into one bet.
...
`import_batch_id` and `source_row` — a timestamp-free bet must always
have a stable, deterministic identity; it is never allowed to fall back
to the non-reproducible ULID token.
...
A normal row only needs `gameDate`, a way to identify the game (`away`/
`home` or `matchup`), enough market description to resolve or an exact
`marketTicker`, `stake`, and `entryPrice` or `entryOdds`. See the
script's module docstring for the full row shape.
