...
**Phase 7 update:** `risk_gate.py` — the Execution Layer's first converted stage — is now also converted (`evaluate_candidate_tt_risk`/`build_risk_portfolio`, pure decision functions called by the still-mutating `apply_tt_safety`/`apply_portfolio_rules` shells — see `docs/IMMUTABLE_PIPELINE.md` §11 for the full before/after), plus a new canonical `data/pipeline/<date>/execution.json` artifact and migration of its own `data/slate.json`/`data/meta.json` writes onto the shared `lib/atomic_json.write_json_atomic()` helper.

**Phase 8 update:** `validate_slate_final.py` — the Recommendation→Validation boundary, the last hard gate before the Execution Layer runs at all — is now also converted (`validate_final_pure`/`_validate_games_pure`/`_route_games_into_slip_buckets`/`build_execution_slip_pure`, pure functions called by the still-printing `validate_final`/`generate_execution_slip` shells — see `docs/IMMUTABLE_PIPELINE.md` §12 for the full before/after), plus a new canonical `data/pipeline/<date>/validation.json` artifact and migration of its `execution_slip_<date>.json`/`data/slate.json`-patch writes onto the shared `lib/atomic_json.write_json_atomic()` helper.

**Phase 9 update:** `protect_slate.py` — the slate-protection gate (sentinel-price hard rejection + official/recheck/rejected run-type routing) — is now also converted (`evaluate_date_mismatch_pure`/`evaluate_sentinel_gate_pure`/`should_sync_legacy_slate_json_pure`, pure functions called at the exact same points in the still-printing `main()`'s original statement order — see `docs/IMMUTABLE_PIPELINE.md` §13 for the full before/after), plus a new canonical `data/pipeline/<date>/protection.json` artifact. Unlike every prior conversion target, `protect_slate.py` was already a thin, 149-line orchestration script — its actual protection logic (sentinel scanning, run-type detection, authoritative-slate merge/freeze rules) lives in the shared, out-of-scope `lib/slate_manager.py` and `lib/sentinel_validator.py`, so Phase 9's pure-extraction surface is correspondingly smaller than prior phases'.

**Phase 10 update (final architecture phase):** `write_pending_bets.py` — the Execution Layer's write/logging boundary, turning `risk_gate.py`'s already-computed Accepted/real-money decisions into durable `bets.json` entries — is now also converted (`should_skip_excluded_game_pure`/`should_block_game_for_pregame_gate_pure`/`is_real_money_market_entry_pure`, pure predicates extracted verbatim from their original inline conditions at the exact same points in `main()`'s two nested loops — see `docs/IMMUTABLE_PIPELINE.md` §14 for the full before/after). `stable_key`, `american_to_decimal_entry`, and `build_bet_record` were already pure before this phase and remain byte-for-byte unchanged. **Zero** of the original ten `data/slate.json` mutators remain unconverted: the sole item Phase 9 left open, `build_market_ledger.py`'s `evaluate_game()` internals, is explicitly out of scope for this phase too (it is market-selection/recommendation logic, which the mission explicitly excludes) — see `docs/IMMUTABLE_PIPELINE.md` §15 for the remaining path. Phase 10 also performed the field-level `authoritative.json`/`execution.json`/`data/slate.json` ownership reconciliation deferred by every prior phase — see `docs/SOURCE_OF_TRUTH_MAP.md` for the final map. Production behavior is unchanged across all eight phases: every conversion and instrumentation was proven behavior-identical by regression tests (golden-baseline tests run against the pre-refactor implementation, then re-run unchanged after) before being considered done. **This is the final phase of the incremental immutable-architecture conversion effort** — see `docs/IMMUTABLE_PIPELINE.md` §15 for why no Phase 11 is recommended.
...
**Also pure (Phase 8 additions):** `scripts/validate_slate_final.py`'s `_diagnostic_lines_pure(games, exp_date)`, `_validate_games_pure(games)`, `validate_final_pure(slate, exp_date)`, `_route_games_into_slip_buckets(games, current_utc)`, `_fmt_real_money_entry(e)`, `_format_slip_lines(...)`, and `build_execution_slip_pure(games, exp_date, current_utc)` — each proven via the same monkeypatch booby-trap technique plus AST-based static scanning for forbidden calls (`open`/`print`/`sys.exit`/`time.sleep`/`socket.socket`/`os.system`/`subprocess.*`), and via explicit identity/mutation-after-return tests. `build_validation_artifact_payload(games, exp_date, errors, warnings)` is also pure — it reshapes an already-computed `(errors, warnings)` pair, never re-running validation.

**Also pure (Phase 9 additions):** `scripts/protect_slate.py`'s `evaluate_date_mismatch_pure(slate_data, expected_date)`, `evaluate_sentinel_gate_pure(sentinels)`, and `should_sync_legacy_slate_json_pure(run_type, auth_path_exists)` — the same booby-trap/AST technique, plus a direct verification (both a frozen legacy snapshot and the current implementation raise the identical `AttributeError` on non-dict `slate.json` input) that extracting these functions at the exact original call-site order preserved even the exception-origination point on malformed input. `build_protection_artifact_payload(date_str, run_type, sentinels, result, synced, auth_exists)` is also pure — it reshapes values `main()` already computed, never re-running protection logic. `_strip_sentinel_metadata()` (pre-existing) was already pure.

**Also pure (Phase 10 additions):** `scripts/write_pending_bets.py`'s `should_skip_excluded_game_pure(game)`, `should_block_game_for_pregame_gate_pure(game_status_result)`, and `is_real_money_market_entry_pure(entry)` — the same booby-trap/AST technique (extended this phase to also trap `os.chdir`, `pathlib.Path`, and `logging.*`), plus explicit non-mutation and determinism proofs. `stable_key(date, game, market, ticker)`, `american_to_decimal_entry(american)`, and `build_bet_record(date, game, entry, now_ts)` were already pure before this phase (no I/O, no clock, no mutation) and are byte-for-byte unchanged — this phase only added test coverage proving that.

**Still mutating, and the remaining candidates for a "pure transform + explicit write" refactor** (each currently reads a file, mutates a dict in place, and writes it back — future-phase work, not attempted here; `enrich_data.py`, `build_market_ledger.py`, `risk_gate.py`, `validate_slate_final.py`, and `protect_slate.py` additionally now publish one or more immutable snapshot artifacts alongside their unchanged mutation, per `docs/IMMUTABLE_PIPELINE.md` §3): `scripts/enrich_data.py`, `scripts/build_market_ledger.py` (its `evaluate_game()` internals specifically — the projection-consumption seam is now pure-adjacent, but the ~1250 lines of edge/confidence/gate logic itself was not restructured — explicitly out of scope for Phase 10 too, since it is market-selection/recommendation logic). `scripts/risk_gate.py`, `scripts/validate_slate_final.py`, `scripts/protect_slate.py`, and `scripts/write_pending_bets.py` moved out of this list in Phases 7, 8, 9, and 10 respectively in the sense that their DECISION/VALIDATION/PROTECTION/LOGGING LOGIC is now pure (see above) — their mutating shells and `main()` functions still mutate `data/slate.json`/`authoritative.json`/`bets.json` in place and write them back, unchanged from before, per the mission's "non-mutating where possible" (not "non-mutating") framing. `write_pending_bets.py` in particular never actually mutated `data/slate.json` (it only reads it) — its own write target, `bets.json`, is append-only by construction, so "mutation" here means only "rewrites the whole file via `json.dump`," not "mutates an existing dict in place."
