...
**Phase 9 update:** `protect_slate.py` — the slate-protection gate (sentinel-price hard rejection + official/recheck/rejected run-type routing) — is now also converted (`evaluate_date_mismatch_pure`/`evaluate_sentinel_gate_pure`/`should_sync_legacy_slate_json_pure`, pure functions called at the exact same points in the still-printing `main()`'s original statement order — see `docs/IMMUTABLE_PIPELINE.md` §13 for the full before/after), plus a new canonical `data/pipeline/<date>/protection.json` artifact. Unlike every prior conversion target, `protect_slate.py` was already a thin, 149-line orchestration script — its actual protection logic (sentinel scanning, run-type detection, authoritative-slate merge/freeze rules) lives in the shared, out-of-scope `lib/slate_manager.py` and `lib/sentinel_validator.py`, so Phase 9's pure-extraction surface is correspondingly smaller than prior phases'. **One** of the original ten mutators (`build_market_ledger.py`'s own `evaluate_game()` internals, which still mutate `marketLedger` in place even though its projection input is now single-sourced) remains unconverted — see `docs/IMMUTABLE_PIPELINE.md` §5 for why it was deferred and §14 for the remaining path and recommended Phase 10. Production behavior is unchanged across all seven phases: every conversion and instrumentation was proven behavior-identical by regression tests (golden-baseline tests run against the pre-refactor implementation, then re-run unchanged after) before being considered done.
...
**Still mutating, and the remaining candidates for a "pure transform + explicit write" refactor** (each currently reads a file, mutates a dict in place, and writes it back — future-phase work, not attempted here; `enrich_data.py`, `build_market_ledger.py`, `risk_gate.py`, `validate_slate_final.py`, and `protect_slate.py` additionally now publish one or more immutable snapshot artifacts alongside their unchanged mutation, per `docs/IMMUTABLE_PIPELINE.md` §3): `scripts/enrich_data.py`, `scripts/build_market_ledger.py` (its `evaluate_game()` internals specifically — the projection-consumption seam is now pure-adjacent, but the ~1250 lines of edge/confidence/gate logic itself was not restructured), `scripts/write_pending_bets.py`. `scripts/risk_gate.py`, `scripts/validate_slate_final.py`, and `scripts/protect_slate.py` moved out of this list in Phases 7, 8, and 9 respectively in the sense that their DECISION/VALIDATION/PROTECTION LOGIC is now pure (see above) — their mutating shells and `main()` functions still mutate `data/slate.json`/`authoritative.json` in place and write them back, unchanged from before, per the mission's "non-mutating where possible" (not "non-mutating") framing.
