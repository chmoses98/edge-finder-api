...
**Phase 6 update:** `post_fetch_gate.py` is now also converted to the pure-transform pattern (`evaluate_game_pitcher_savant`/`evaluate_game_team_stats`/`apply_post_fetch_gate_immutable` — see `docs/IMMUTABLE_PIPELINE.md` §4), and its quarantine-marker write hardened to the same atomic pattern via a newly-extracted shared helper, `lib/atomic_json.write_json_atomic()`, which `fetch_lineups.py` and `fetch_savant_pitchers.py` also now delegate to instead of each keeping its own inlined copy. Separately, `build_market_ledger.py`'s `compute_projections()` is now called exactly once per game per run (via the new `compute_game_projection_context()`), with the same result threaded explicitly into both the `projections.json` artifact and `evaluate_game()` (which gained an optional, backward-compatible `projection_context` parameter) — closing the "two independent calls that happened to agree" gap Phase 4 left open. `evaluate_game()`'s own ~1250 lines of edge/confidence/gate logic were not otherwise touched. The Normalized Slate layer (§2/§7 of `docs/IMMUTABLE_PIPELINE.md`) is now **fully** converted. **Four** of the original ten mutators (`validate_slate_final.py`, `protect_slate.py`, `risk_gate.py`, plus `build_market_ledger.py`'s own `evaluate_game()` internals, which still mutate `marketLedger` in place even though its projection input is now single-sourced) remain unconverted — see `docs/IMMUTABLE_PIPELINE.md` §5 for why each was deferred, §7 for the recommended Phase 7 sequencing, and §11 specifically for why `risk_gate.py` stays out of scope. Production behavior is unchanged across all four phases: every conversion and instrumentation was proven behavior-identical by regression tests (golden-baseline tests run against the pre-refactor implementation, then re-run unchanged after) before being considered done.
...
**Still mutating, and the remaining candidates for a "pure transform + explicit write" refactor** (each currently reads a file, mutates a dict in place, and writes it back — Phase 7+ work, not attempted here; `enrich_data.py` and `build_market_ledger.py` additionally now publish one or more immutable snapshot artifacts alongside their unchanged mutation, per `docs/IMMUTABLE_PIPELINE.md` §3): `scripts/enrich_data.py`, `scripts/build_market_ledger.py` (its `evaluate_game()` internals specifically — the projection-consumption seam is now pure-adjacent, but the ~1250 lines of edge/confidence/gate logic itself was not restructured), `scripts/risk_gate.py`, `scripts/validate_slate_final.py`, `scripts/protect_slate.py`, `scripts/write_pending_bets.py`.
