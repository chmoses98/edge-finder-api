...
**Still mutating, and the remaining candidates for a "pure transform + explicit write" refactor** (each currently reads a file, mutates a dict in place, and writes it back — future-phase work, not attempted here; `enrich_data.py`, `build_market_ledger.py`, `risk_gate.py`, and `validate_slate_final.py` additionally now publish one or more immutable snapshot artifacts alongside their unchanged mutation, per `docs/IMMUTABLE_PIPELINE.md` §3): `scripts/enrich_data.py`, `scripts/build_market_ledger.py` (its `evaluate_game()` internals specifically — the projection-consumption seam is now pure-adjacent, but the ~1250 lines of edge/confidence/gate logic itself was not restructured), `scripts/protect_slate.py`, `scripts/write_pending_bets.py`. `scripts/risk_gate.py` and `scripts/validate_slate_final.py` moved out of this list in Phases 7 and 8 respectively in the sense that their DECISION/VALIDATION LOGIC is now pure (see above) — their mutating shells and `main()` functions still mutate `data/slate.json` in place and write it back, unchanged from before, per the mission's "non-mutating where possible" (not "non-mutating") framing.
