...
**Phase 4 update:** `merge_odds.py` is now also converted to the pure-transform pattern (`compute_game_odds_fields`/`merge_odds_immutable` — see `docs/IMMUTABLE_PIPELINE.md` §4), and `build_market_ledger.py` additionally publishes a third artifact, `data/pipeline/<date>/projections.json`, snapshotting `compute_projections()`'s output ahead of any recommendation decision — a narrowed, canonical schema rather than another full-slate transitional dump (`docs/IMMUTABLE_PIPELINE.md` §3). **Seven** of the original ten mutators (`fetch_lineups.py`, `post_fetch_gate.py`, `fetch_savant_pitchers.py`, `validate_slate_final.py`, `protect_slate.py`, `risk_gate.py`, plus `build_market_ledger.py`'s own `evaluate_game()` internals, which still mutate `marketLedger` in place even though its projection input is now separately snapshotted) remain unconverted — see `docs/IMMUTABLE_PIPELINE.md` §5 for why each was deferred, §7 for the recommended Phase 5 sequencing, and §11 specifically for why `risk_gate.py` stays out of scope. Production behavior is unchanged in both phases: every conversion and instrumentation was proven behavior-identical by regression tests (golden-baseline tests run against the pre-refactor implementation, then re-run unchanged after) before being considered done.
...
**Converted to pure-transform:** `scripts/enrich_lineup_confirmed.py` (Phase 3) and `scripts/merge_odds.py` (Phase 4) — see `docs/IMMUTABLE_PIPELINE.md` §4. Each script's core per-game logic now lives in pure functions (`compute_game_lineup_fields`/`enrich_games_immutable` for the former; `compute_game_odds_fields`/`merge_odds_immutable` for the latter) that build and return new objects rather than mutating their inputs; each script's file I/O wrapper (`main()`, or the top-level script body for `merge_odds.py`, which has no `main()`) is unchanged in behavior.

**Still mutating, and the remaining candidates for a "pure transform + explicit write" refactor** (each currently reads a file, mutates a dict in place, and writes it back — Phase 5+ work, not attempted here; `enrich_data.py` and `build_market_ledger.py` additionally now publish one or more immutable snapshot artifacts alongside their unchanged mutation, per `docs/IMMUTABLE_PIPELINE.md` §3): `scripts/enrich_data.py`, `scripts/build_market_ledger.py`, `scripts/risk_gate.py`, `scripts/fetch_lineups.py`, `scripts/protect_slate.py`, `scripts/write_pending_bets.py`.
