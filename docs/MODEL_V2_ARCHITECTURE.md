...
**Phase 5 update:** `fetch_lineups.py` and `fetch_savant_pitchers.py` are now also converted to the pure-transform pattern (network adapter → pure parser → pure per-game transform → pure per-slate transform — see `docs/IMMUTABLE_PIPELINE.md` §4), and both scripts' `data/slate.json` write was hardened to an atomic temp-file+`os.replace()` pattern (a pure reliability fix, verified byte-identical output in the success path). **Five** of the original ten mutators (`post_fetch_gate.py`, `validate_slate_final.py`, `protect_slate.py`, `risk_gate.py`, plus `build_market_ledger.py`'s own `evaluate_game()` internals, which still mutate `marketLedger` in place even though its projection input is now separately snapshotted) remain unconverted — see `docs/IMMUTABLE_PIPELINE.md` §5 for why each was deferred, §7 for the recommended Phase 6 sequencing, and §11 specifically for why `risk_gate.py` stays out of scope. Production behavior is unchanged across all three phases: every conversion and instrumentation was proven behavior-identical by regression tests (golden-baseline tests run against the pre-refactor implementation, then re-run unchanged after) before being considered done.
...
**Converted to pure-transform:** `scripts/enrich_lineup_confirmed.py` (Phase 3), `scripts/merge_odds.py` (Phase 4), and `scripts/fetch_lineups.py` / `scripts/fetch_savant_pitchers.py` (Phase 5) — see `docs/IMMUTABLE_PIPELINE.md` §4. Each script's core per-game logic now lives in pure functions (`compute_game_lineup_fields`/`enrich_games_immutable`; `compute_game_odds_fields`/`merge_odds_immutable`; `parse_lineup_response`/`compute_game_lineup_stats_fields`/`apply_lineups_immutable`; `compute_pitcher_savant_enrichment`/`sanitize_recent_fip`/`compute_game_pitcher_savant_fields`/`apply_savant_enrichment_immutable`) that build and return new objects rather than mutating their inputs; each script's file I/O wrapper (`main()`, or the top-level script body for `merge_odds.py`, which has no `main()`) is unchanged in behavior. `fetch_lineups.py` and `fetch_savant_pitchers.py` additionally gained their own network adapters (`fetch_boxscore`; `fetch_json`/`fetch_batch`, the latter pair unchanged from before) explicitly separated from the pure parse/transform layer.

**Still mutating, and the remaining candidates for a "pure transform + explicit write" refactor** (each currently reads a file, mutates a dict in place, and writes it back — Phase 6+ work, not attempted here; `enrich_data.py` and `build_market_ledger.py` additionally now publish one or more immutable snapshot artifacts alongside their unchanged mutation, per `docs/IMMUTABLE_PIPELINE.md` §3): `scripts/enrich_data.py`, `scripts/build_market_ledger.py`, `scripts/risk_gate.py`, `scripts/post_fetch_gate.py`, `scripts/protect_slate.py`, `scripts/write_pending_bets.py`.
