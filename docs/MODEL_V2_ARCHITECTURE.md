...
**Phase 3 update:** this is no longer entirely unattempted. `docs/IMMUTABLE_PIPELINE.md` documents the first incremental step: `enrich_lineup_confirmed.py` was converted from in-place mutation to a pure-transform pattern (builds and returns a new game/slate object rather than mutating the loaded one), and `enrich_data.py`/`build_market_ledger.py` now additionally publish immutable snapshot artifacts (`data/pipeline/<date>/normalized_slate.json` and `.../recommendations.json`) at the Normalized Slate and Recommendation Layer boundaries, alongside their unchanged legacy `data/slate.json` write. The other eight mutators (`fetch_lineups.py`, `post_fetch_gate.py`, `fetch_savant_pitchers.py`, `merge_odds.py`, `validate_slate_final.py`, `protect_slate.py`, `risk_gate.py`) are still unconverted — see `docs/IMMUTABLE_PIPELINE.md` §5 for why each was deferred and §7 for the recommended Phase 4 sequencing. Production behavior is unchanged: every conversion and instrumentation this phase was proven behavior-identical by regression tests before being considered done.
...
**Converted to pure-transform this phase (Phase 3):** `scripts/enrich_lineup_confirmed.py` — see `docs/IMMUTABLE_PIPELINE.md` §4. Its core logic now lives in two pure functions (`compute_game_lineup_fields`, `enrich_games_immutable`) that build and return new objects rather than mutating their inputs; `main()`'s file I/O wrapper is unchanged in behavior.

**Still mutating, and the remaining candidates for a "pure transform + explicit write" refactor** (each currently reads a file, mutates a dict in place, and writes it back — Phase 4+ work, not attempted here; `enrich_data.py` and `build_market_ledger.py` additionally now publish an immutable snapshot artifact alongside their unchanged mutation, per `docs/IMMUTABLE_PIPELINE.md` §3): `scripts/enrich_data.py`, `scripts/merge_odds.py`, `scripts/build_market_ledger.py`, `scripts/risk_gate.py`, `scripts/fetch_lineups.py`, `scripts/protect_slate.py`, `scripts/write_pending_bets.py`.
