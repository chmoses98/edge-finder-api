...
| 3 | **`workflow_dispatch`** | a human asks | an explicit `date`, or an inclusive `start_date`/`end_date` range, or the lookback sweep; `dry_run: true` computes and writes nothing |
