...
Wraps lib.kalshi_market_coverage.build_coverage_ledger/coverage_accounting
around the SAME data scripts/discover_kalshi_mlb_markets.py already reads
(data/kalshi_search.json + data/slate.json for the given date) -- this
guarantees the coverage ledger and the discovery artifact describe the
exact same archived Kalshi market observation, never a separately-fetched
or re-classified copy (docs/KALSHI_MLB_MARKET_COVERAGE_AUDIT.md Phase 5:
"ensure both use the same archived market universe for the same
observation").
...
contract, whether it was fully evaluated or has an explicit, named reason
it wasn't -- never a silent gap.
...
Exit code is non-zero if unaccountedCount > 0 for the date's coverage
ledger -- the CI-enforced form of the "zero silent omissions" invariant
(docs/KALSHI_MLB_MARKET_COVERAGE_AUDIT.md Phase 7).
"""
import json
import os
import sys
...
def print_ledger_report(date_str, accounting):
    by_state = accounting["byState"]
    print(f"\nFULL MARKET COVERAGE REPORT -- {date_str}")
    print(f"  Archived total : {accounting['archivedTotal']}")
    print(f"  Accounted for  : {accounting['accountedTotal']}")
    print(f"  Unaccounted    : {accounting['unaccountedCount']}  <- must be 0")
    for state in sorted(by_state):
        print(f"    {state:26s}: {by_state[state]}")
...
def main(date_str=None, search_path=None, slate_path=None, out_dir=None, dry_run=False):
    ...
    ledger_rows, discovery_summary = build_coverage_ledger(date_str, search_doc, slate_doc)
    accounting = coverage_accounting(ledger_rows)

    payload = {
        "date": date_str,
        "discoverySummary": discovery_summary,
        "accounting": {k: v for k, v in accounting.items() if k != "byFamilyState"},
        "byFamilyState": accounting["byFamilyState"],
    }
    ...
    print_ledger_report(date_str, accounting)
...
if __name__ == "__main__":
    ...
    unaccounted = result.get("accounting", {}).get("unaccountedCount", 0)
    if result.get("status") == "OK" and unaccounted > 0:
        print(f"\nFAIL: {unaccounted} archived market(s) unaccounted for -- coverage invariant violated.", file=sys.stderr)
        sys.exit(1)
    sys.exit(0)
