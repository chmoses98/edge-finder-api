...
IMPORT SAFETY (WAVE 0.05A)
--------------------------
Everything below lives inside a function. Importing this module defines
names and does nothing else -- it does not read the slate, print, or
terminate the interpreter.

It used to be a flat script: the checks ran at module scope and the file
ended in a bare `sys.exit(...)`. Because the filename matches pytest's
default `*_test.py` collection glob, `python3 -m pytest` from the repo
root imported it during collection, and the module-scope `sys.exit`
aborted the whole session:

    INTERNALERROR> SystemExit: 1
    mainloop: caught unexpected SystemExit!
    ============================ no tests ran =============================

pytest exits 3 there, so it is not a silent success -- but it collects
and runs ZERO tests, which is why the canonical suite has to be scoped
`python3 -m pytest tests/` (pr-ci.yml). A contributor running bare
pytest got no coverage at all.

The rule this file now follows: importing a production or utility module
may define functions, constants and classes; it must never execute the
CLI or terminate the interpreter.
"""
...
DEFAULT_SLATE_PATH = 'data/slate.json'


def check_slate(slate):
    """
    Pure. Runs assertions A1-A9 over an already-loaded slate object and
    returns (fails, totals, games). Reads nothing, writes nothing, exits
    nothing -- so it is safe to call from a test.
    """
    ...
    return fails, totals, games


def main(argv=None, slate_path=DEFAULT_SLATE_PATH):
    """
    The CLI. Output text and exit codes are byte-for-byte what the flat
    script produced: 1 when the slate is absent or any assertion fails,
    0 otherwise.
    """
    ...
    fails, totals, games = check_slate(slate)
    expected_total = len(games) * len(REQUIRED_MARKETS)
    actual_total   = sum(totals.values())
