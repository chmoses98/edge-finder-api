#!/usr/bin/env python3
"""
tests/test_research_multiseason_starter_workload_backtest_workflow_guard.py
================================================================================
...
(MLB-RSCH-0003's own workflow), applied to MLB-RSCH-0004. Both the
protected-branch guard and the test-dependency install step are present
from this workflow's first version (learned from MLB-RSCH-0003's first
real dispatch, which failed on a missing `pytest` install) -- this test
proves both are actually there and actually work, not merely present in
the YAML.
"""
import os
import subprocess
...
WORKFLOW_PATH = os.path.join(ROOT, ".github", "workflows", "research-multiseason-starter-workload-backtest.yml")
...
class TestGuardBehavior:
    ...
    def test_allows_a_research_branch(self, guard_step):
        result = _run_guard_script(guard_step, "claude/mlb-rsch-0004-starter-workload", "main")
        assert result.returncode == 0


def test_dependency_install_step_exists_before_pytest_step(steps):
    names = [s.get("name", "") for s in steps]
    pytest_step_index = next(i for i, s in enumerate(steps) if "python3 -m pytest" in (s.get("run") or ""))
    ...
    assert install_step_index < pytest_step_index
...
def test_commit_step_scoped_to_this_experiments_own_paths(steps):
    commit_step = next(s for s in steps if "commit cache" in (s.get("name") or "").lower())
    run_script = commit_step.get("run") or ""
    assert "data/research_cache/starter_workload/" in run_script
    assert "MLB-RSCH-0004" in run_script
    assert "data/research_cache/bullpen_backtest/" not in run_script
